...
with data_space:
    ...
    st.text(
        f'현재 수집되고 있는 언론사는 조선일보, 중앙일보, 동아일보, 한경, 매경, 연합뉴스 대상으로 수집된 전체 수집된 기사 중 \n유의미 하다고 판단된 기사는 총 {df.shape[0]}건이며, 금일 기준 ({df.날짜.max()}) 수집된 주요 기사는 총 {df[df.날짜 == df.날짜.max()].shape[0]}건 입니다.')
    st.text('')
    ''
